...
namespace SmartHomeUI {
    ...
    public:
        ...
        vector<shared_ptr<ICommand>>* availableCommands;
        map<string, vector<shared_ptr<ICommand>>>* deviceCommandsMap;
        ...
        SmartHomeData()
        {
            ...
            deviceCommandsMap = new map<string, vector<shared_ptr<ICommand>>>();
        }
    ...
    private:
        ...
        void InitializeSmartHome()
        {
            ...
            smartHome->remote = new RemoteControl();
            smartHome->availableCommands = new vector<shared_ptr<ICommand>>();
        ...
        void CreateAvailableCommands()
        {
            ...
            for (auto& room : smartHome->house->getRooms()) {
                ...
                if (roomPtr) {
                    ...
                    auto roomTurnOn = make_shared<TurnOnCommand>(room);
                    auto roomTurnOff = make_shared<TurnOffCommand>(room);
                    smartHome->availableCommands->push_back(roomTurnOn);
                    smartHome->availableCommands->push_back(roomTurnOff);

                    // Команды для каждого устройства
                    for (auto& device : roomPtr->getDevices()) {
                        auto deviceTurnOn = make_shared<TurnOnCommand>(device);
                        auto deviceTurnOff = make_shared<TurnOffCommand>(device);
                        smartHome->availableCommands->push_back(deviceTurnOn);
                        smartHome->availableCommands->push_back(deviceTurnOff);
                    }
                }
            }
        }
        ...
        void BuildDeviceCommandsMap()
        {
            ...
            for (auto& cmd : *(smartHome->availableCommands)) {
                ...
                if (pos != string::npos) {
                    ...
                    (*smartHome->deviceCommandsMap)[deviceName].push_back(cmd);
                }
            }
        }
        ...
        void OnAddDeviceClick(Object^ sender, EventArgs^ e)
        {
            ...
            // Проверяем, что выбран узел комнаты (узел с уровнем 2: корень->комната->устройство)
            ...
            if (result == System::Windows::Forms::DialogResult::Yes) {
                ...
                for (auto& room : smartHome->house->getRooms()) {
                    if (room->getName() == roomNameStr) {
                        Room* roomPtr = dynamic_cast<Room*>(room.get());
                        if (roomPtr) {
                            // Находим устройство по имени и создаем shared_ptr для удаления
                            for (auto& device : roomPtr->getDevices()) {
        ...
        void OnDeviceSelected(Object^ sender, TreeViewEventArgs^ e)
        {
            ...
            if (selectedNode->Level == 2) {
                ...
                if (deviceName != nullptr) {
                    string devName = msclr::interop::marshal_as<string>(deviceName);

                    // Показываем команды только для выбранного устройства
                    if (smartHome->deviceCommandsMap->find(devName) != smartHome->deviceCommandsMap->end()) {
                        for (auto& cmd : (*smartHome->deviceCommandsMap)[devName]) {
                            String^ cmdName = gcnew String(cmd->getName().c_str());
                ...
                for (auto& cmd : *(smartHome->availableCommands)) {
                    string cmdName = cmd->getName();
                    if (cmdName.find(roomNameStr) != string::npos) {
                        String^ cmdNameStr = gcnew String(cmdName.c_str());
                        lbCommands->Items->Add(cmdNameStr);
                    }
                }
            }
            else {
            ...
            if (lbCommands->SelectedIndex >= 0) {
                ...
                for (auto& cmd : *(smartHome->availableCommands)) {
                    if (cmd->getName() == selectedCmdStr) {
                        smartHome->remote->pressButton(cmd);
                        // Обновляем дерево устройств для отображения нового статуса
                        UpdateDeviceTree();
